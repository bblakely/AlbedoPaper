...
nochange<- length(which(ModernVeg$B1==PaleoVeg$B1))
...
convert.code.split.def<-convert.code.split[TotDecidify]
list.ind.split.dec<-list()
for(i in 1:length(poss)){
  list.ind.split.dec[[i]]<- which(convert.code.split.def==poss[i])
}
