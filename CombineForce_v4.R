...
rm(list=setdiff(ls(), c("alb.force", "st.force","alb.diff","st.diff", "veg.force",
                        ...
                        'uncertainty.comp.st.temp','STChg.def.avg', 'uncertainty.def.st.temp', 'reportnum')))
...
axis(side=4, labels=seq(from=-3, to=1, by=1), at=seq(from=-3, to=1, by=1), cex.axis=2.2, font=2)
mtext(side=4, text=ylab, line=4, cex=2.4, font=2)
abline(h=0, lwd=3)
box(lwd=3)
...
barplot(c(Tot.ann, veg.comb,sno.comb),col=c("black","gray","dark blue"),  
        main="Combined RF",names.arg="", font=2, #names.arg=c("Combined","Vegetation","Snow")
        ylim=ylim,cex.main=5, cex.axis=4.5,cex.names=4, ylab='',
        cex.lab=4, font.axis=2, font.lab=2, axes=FALSE)

abline(h=seq(from=-4,to=3,by=0.5), lty=2,col='light gray')

barplot(c(Tot.ann, veg.comb,sno.comb),col=c("black","gray","dark blue"), 
        main="Combined RF",names.arg="", font=2,
        ylim=ylim,cex.main=5, cex.axis=4.5,cex.names=4, ylab='',
        cex.lab=4, font.axis=2, font.lab=2, axes=FALSE, add=T)

barplot(c(Tot.ann, veg.comb,sno.comb), density=c(0,20,50),col=c("black","forest green","light blue"), 
        main="Combined RF",names.arg="", font=2,
        ylim=ylim,cex.main=5, cex.axis=4.5,cex.names=4, ylab='',
        cex.lab=4, font.axis=2, font.lab=2, axes=FALSE, add=TRUE)

axis(side=1, labels=c("Combined","Vegetation","Snow"), at=barplot(c(Tot.ann, veg.comb,sno.comb), plot=FALSE), tick=FALSE, cex.axis=4, line=3, font=2)
axis(side=2, labels=seq(from=-4, to=3, by=1), at=seq(from=-4, to=3, by=1), cex.axis=4.5, font=2)
mtext(side=2, text=ylab, line=5, cex=5, font=2)
abline(h=0, lwd=3)
box(lwd=5)

dev.copy(tiff, filename="Figures/ComboBars.tif", width=800*8, height=640*8, res=300);dev.off()
...
legend(6.5,-9,legend=c("Deforestation", "Composition"), fill=c('navajowhite1','darkseagreen1'),text.font=2, bty='n', cex=1.2)
...
##Surface Temperature
#setup
lablim=1.4; span<-c(-lablim,lablim)
ylab<-expression(Delta~LST~(degree*C))
plot(STChg.def.avg, type='l', col='orange', ylim=span, lwd=2, cex.main=2.5, ylab="", xlab="",cex.lab=2.1,yaxt='n',xaxt='n',bty='n')
axis(side=1,labels=seq(from=1, to=12, by=2),at=seq(from=1, to=12, by=2), cex.axis=1.5, font=2)
#axis(side=1,labels=c(1:12),at=c(1:12), cex.axis=1.5, font=2)
...
legend(6.5,-0.8,legend=c("Deforestation", "Composition"), fill=c('navajowhite1','darkseagreen1'),text.font=2, bty='n', cex=1.2)

dev.copy(png, filename="Figures/CombinedSTChange.png", width=410, height=350);dev.off()
...
for(i in dest){
  ...
  plot(colMeans(alb.force[convert.code==key$Code[i],],na.rm=TRUE), ylim=c(-30,18), ylab='RF', lty=2,lwd=2, 
  ...
  lines(colMeans(veg.force[convert.code==key$Code[i],], na.rm=TRUE), lwd=3)
    ...
    plot(colMeans(veg.force[convert.code==key$Code[i],],na.rm=TRUE), ylim=c(-30,18), ylab='RF',lwd=2, 
