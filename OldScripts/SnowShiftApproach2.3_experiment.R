...
######
  ...
  Mnorm<-Mpix/max(Epix, na.rm=TRUE)
...
#######  
  ...
  shift.quant<-quantile(shift.avg, .99, na.rm=TRUE)
  ...
  plot(Escale,type='l',lty=2, lwd=2, xlab='Month',
       main='Albedo Shift', col='white', ylim=c(0,.7), pch=1.5,
       ylab='Albdeo', xlim=c(start,end),font=2, font.lab=2)
     
  #originally had main=n and no color=white or ylim to track during dev
  #print(mean(Scalars[3:5]))
  #axis(1,at=seq(from=1, to=12,length.out=12),labels=c(1:12))
  legend(4.4,0.7, legend=c('Modern','Shifted'),
         col=c('purple', 'orange'), lwd=2, text.font=2)
  #lines(Mscale, type='l',col='red', lty=2, lwda=2)
  lines(Apix, col='purple',lty=2, lwd=2)
  lines(HistAlb, col='orange', lwd=2)
  box(lwd=3)
  
  plot(Epix, type='l', main='SWE',xlim=c(start,end), lwd=2,xlab='Month',ylab='SWE', 
       font=2, font.lab=2)
  lines(Mpix, col='red', lwd=2)
  legend(4,y=max(Epix), legend=c('2000 - 2010', '1900 - 1910'),
         col=c('red', 'black'), lwd=2, text.font=2 )
  box(lwd=3)
  
  plot(Enorm, type='l', main='Normalized SWE',xlim=c(start,end), ylab='% Maximum SWE',
        lwd=2,xlab='Month',font=2, font.lab=2)
  lines(Mnorm, col='red', lwd=2)
  legend(4,1, legend=c('2000 - 2010', '1900 - 1910'),
          col=c('red', 'black'), lwd=2, text.font=2)
  box(lwd=3)
